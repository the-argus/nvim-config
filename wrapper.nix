{
  ...
  minimal ? false,
  banner ? null,
  # either an attrset in the banner palette format, or a path to a yaml file.
  # only the 16 ansi colors are used
  bannerPalette ? null,
  zls,
  latestZls ? zls,
  wrapNeovimUnstable,
  ...
  # turn the banner palette into a lua file that contains the variables of the
  # palette in lua form. This means there is lua syntax and hyphens are
  # replaced with underscores. In the config we do require("nix-banner-palette")
  # in order to get the palette colors as a lua table
  palettePlugin = let
    palette =
      if builtins.isAttrs bannerPalette
      then bannerPalette
      else banner.lib.parsers.basicYamlToBanner bannerPalette;

    paletteVariablesAsLua = lib.attrsets.mapAttrsToList (
      name: value: let
        luaName = builtins.replaceStrings ["-"] ["_"] name;
        hex = lib.strings.removePrefix "#" value;
      in ''${luaName} = "#${hex}",''
    ) (banner.lib.util.removeMeta palette);
  in
    pkgs.writeTextDir "lua/nix-banner-palette.lua" ''
      -- generated by nix
      return {
      ${lib.concatStringsSep "\n" paletteVariablesAsLua}
      }
    '';

  # needs to be optional to avoid evaluating palettePlugin when no bannerPalette
  # was specified
  paletteConfig = lib.optionalString (
    bannerPalette != null
  ) "vim.opt.runtimepath:prepend('${palettePlugin}')";
