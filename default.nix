...
with lib;
  neovim: let
    wrapper = {
      ...
      wrapperArgs ? [],
      # this is all of the packages (probably LSPs) that neovim needs
      wapperBin ? [],
      # plugins to be in the lua path for neovim
      plugins ? [],
      manifestRc ? null,
      withPython2 ? false,
      withPython3 ? true,
      ...
      wrapperBinPath =
        if isString wrapperBin
        then wrapperBin
        else makeBinPath wrapperBin;

      wrapperArgsStr =
        if isString wrapperArgs
        then wrapperArgs
        ...
        symlinkJoin {
          ...
          postBuild =
            ...
            + optionalString (manifestRc != null) (let
              manifestWrapperArgs = ["${neovim}/bin/nvim" "${placeholder "out"}/bin/nvim-wrapper"];
            in ''
